...
classdef three_axis_sensor_model_1
    ...
    properties (SetAccess = 'public')
        ...
        DRN = [0;0;0];  % Random Drift = ND * sqrt(BW)
        DRB = [0;0;0];  % Random Drift attributable to Bias
        DRK = [0;0;0];  % Random Drift rate attributable to Rate Random Walk
        DRR = [0;0;0];  % Random Drift attributable to ramp
    ...
    methods
        function s = three_axis_sensor_model_1(range, NB, Ts)
        ...
        function s = set_random_walk(s, DRK, tau)
            ...
            s.DRK = DRK;
            s.DRK_input = [ sqrt(2*DRK(1)^2/(s.Ts*tau)); ...
        ...
        function [vectorOut] = corrupt(s, temperature, time, vectorIn, otherVectorIn)
            % Bulk Model
            B = s.G*vectorIn + s.O;
            % Drift Model
            % First calculate drift due to rate random walk
            wk = s.DRK_input.*randn(3,1);
            s.DRK = s.Ts*wk + s.DRK - (s.Ts/s.Tau)*s.DRK;
            DRR = s.DRR * time;         % Random drift attributable to ramp
            DRN = s.DRN .* randn(3,1);  % Random drift attributable to angle random walk
            DRB = s.DRB .* randn(3,1);  % Random drift attributable to bias
